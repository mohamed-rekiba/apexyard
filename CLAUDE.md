...
| Rules | `.claude/rules/` | 20 modular rule files (AgDR triggers, agent role selection, code standards, git conventions, glossary lookup, isolated builds, leak protection, loop mode, parallel work, plan mode, PR quality, PR workflow, README quality, reconcile before build, reporting style, right-size ceremony, role triggers, skill first, ticket vocabulary, workflow gates) |
