...
11. **Active-issue-skill marker handling (defensive).** Write `plan-initiative` to `<ops_root>/.claude/session/active-issue-skill` before the first `tracker_create` in pass 1; remove on EVERY exit path (success, abort, error). Per AgDR-0030. Issue creation runs through the `tracker_create` **function** via command substitution (`result="$(tracker_create …)"`), which `require-skill-for-issue-create.sh` doesn't fire on (#709/#771), so the marker is a defensive backstop kept for robustness (and any path that still shells out to a bare `gh issue create` at a command boundary), not the mechanism that permits the create.
12. **No bootstrap exemption needed.** The skill writes `*.md` (exempt from `require-active-ticket.sh`) and dispatches issue creation through the `tracker_create` **function** via command substitution, which `require-skill-for-issue-create.sh` doesn't fire on (#709/#771; the active-issue-skill marker written in step 8 pass 1 is a defensive backstop, not the thing that satisfies the gate). Don't add `plan-initiative` to `ticket.bootstrap_skills`.
