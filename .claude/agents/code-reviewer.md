...
The marker MUST land at `<ops_fork_root>/.claude/session/reviews/<owner>__<repo>__<pr>-rex.approved`, and you MUST get that path from `review_marker_path` rather than composing it by hand.

**Never hand-write the filename.** This paragraph previously showed the bare `{number}-rex.approved` form, contradicting the repo-qualified scheme stated at the top of this file and implemented in `_lib-review-markers.sh` (AgDR-0060). An orchestrator following the stale form wrote a valid approval to `23-rex.approved` while the merge gate looked for `owner__repo__23-rex.approved`; the review had genuinely happened, the merge was blocked anyway, and the fix cost an extra review spawn purely to relocate a file. Resolve the path, don't retype it (me2resh/apexyard#7).

Inside `workspace/<project>/`, `git rev-parse --show-toplevel` returns the project clone — NOT the ops fork. Writing to a relative `.claude/session/reviews/` path from inside a workspace clone puts the marker where the merge-gate hook can't see it (the bug fix in me2resh/apexyard#229 + #230 aligned the merge gate with this path; this section is the agent-side counterpart).
